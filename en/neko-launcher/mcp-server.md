...
Neko Launcher can run a local **MCP (Model Context Protocol) server**, so an AI client such as Claude Code, Claude Desktop, Cursor or Codex can list and launch instances, read logs, manage mods and build local instances for you.

## Turn it on

1. Open **Settings → Developer → MCP server** and switch it on. It listens on `127.0.0.1` by default.
2. Set an **access key** if anything other than your own PC could reach the port.
3. **Connect a client** on the same page gives the exact command or config for each client. The quickest way is the CLI:

```bash
npx @neko-launcher/cli setup claude-code
```

**Settings → Developer → MCP tools** lists every tool the server offers, searchable, straight from the server itself.

## Tools

The launcher currently offers **76 tools**. Tools marked *DESTRUCTIVE* delete data; a good client asks you first.
...
`local_instance_create`, `modpack_import` and `modpack_install` return a `jobId` at once and keep working in the background (an install can take minutes). Poll `local_jobs` for progress and the result, or stop one with `local_job_cancel`. The launcher window shows these jobs like the ones it started itself.

## Not exposed

Window management, native file dialogs, the browser-based Microsoft login, Microsoft access tokens and the access key itself are never offered over MCP.
